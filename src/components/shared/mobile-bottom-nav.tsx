    ...
    <>
          ...
          <section id="mobile-more-options" role="dialog" aria-modal="true" aria-label="Más opciones de navegación" className="absolute inset-x-0 bottom-0 max-h-[calc(100dvh-env(safe-area-inset-top))] overflow-y-auto rounded-t-3xl bg-surface px-5 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] pt-3 shadow-2xl dark:bg-slate-900" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="grid gap-2">
              {moreItems.map((item) => {
                ...
                return <Link key={item.href} href={item.href} onClick={() => setIsMoreOpen(false)} aria-current={isActive ? "page" : undefined} className={`flex items-center gap-3 rounded-2xl px-4 py-3.5 text-sm font-bold transition ${isActive ? "bg-brand-green-soft text-brand-green dark:bg-brand-green/15 dark:text-brand-lime-light" : "text-ink hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"}`}><NavIcon name={item.icon} className="h-5 w-5" />{item.label}</Link>;
              })}
            </div>
          </section>
        </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-border bg-surface pb-[env(safe-area-inset-bottom)] dark:border-slate-800 dark:bg-slate-900 lg:hidden" aria-label="Navegación móvil">
        {visibleItems.map((item) => {
          const isActive = isCurrentRoute(pathname, item.href, item.href === items[0]?.href);
          return <Link key={item.href} href={item.href} aria-current={isActive ? "page" : undefined} className="flex flex-1 flex-col items-center gap-1 py-2.5 text-[11px] font-semibold transition"><span className={`grid h-9 w-9 place-items-center rounded-full transition ${isActive ? "bg-brand-green-soft text-brand-green dark:bg-brand-green/15 dark:text-brand-lime-light" : "text-muted dark:text-slate-500"}`}><NavIcon name={item.icon} className="h-5 w-5" /></span><span className={isActive ? "text-brand-green dark:text-brand-lime-light" : "text-muted dark:text-slate-500"}>{item.shortLabel}</span></Link>;
        })}
        {hasMoreItems && (
          <button type="button" onClick={() => setIsMoreOpen(true)} aria-expanded={isMoreOpen} aria-controls="mobile-more-options" className={`flex flex-1 flex-col items-center gap-1 py-2.5 text-[11px] font-semibold transition ${isMoreActive ? "text-brand-green dark:text-brand-lime-light" : "text-muted dark:text-slate-500"}`}>
            <span className={`grid h-9 w-9 place-items-center rounded-full transition ${isMoreActive ? "bg-brand-green-soft text-brand-green dark:bg-brand-green/15 dark:text-brand-lime-light" : "text-muted dark:text-slate-500"}`}><MoreIcon /></span>
            <span>Más</span>
          </button>
        )}
      </nav>
