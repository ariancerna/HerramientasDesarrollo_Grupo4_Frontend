  ...
  return (
    <div>
      ...
      <section className="mt-6 rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
        <span className="mx-auto grid h-14 w-14 place-items-center rounded-lg bg-primary-soft text-primary-dark"><ModuleIconGraphic name={icono} /></span>
        <h1 className="mt-4 text-2xl font-bold tracking-tight text-ink">{titulo}</h1>
        <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">{descripcion}</p>
        <div className="mt-6 inline-flex rounded-full bg-primary-soft px-3 py-1 text-xs font-semibold text-primary-dark">Módulo en desarrollo</div>
        <div className="mt-8">
          <Link href="/dashboard/admin" className="inline-flex rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-white transition hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">Volver al panel</Link>
        </div>
      </section>
    </div>
  );
